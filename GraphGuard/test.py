...
from util import load_dataset, separate_data, adversarial_train_test
...
def get_model_output(args, model, device, test_graphs, sub_func, **kwargs):
    ...
    if args.use_second_implementation:
        ...
        for i in tqdm(range(0, len(test_graphs), minibatch_size), total=1+len(test_graphs)//minibatch_size):
        ...
        test_graphs = Parallel(n_jobs=-1)(
            ...
            for graph in tqdm(test_graphs, desc='Processing Graphs')
        )

        test_graphs = sum(test_graphs, start=[])
        output = pass_data_iteratively(model, test_graphs, args.batch_size)
        output = output.detach().cpu().numpy()
...
def main():
    ...
    parser.add_argument("--hash-method", type=str, choices=["hash", "md5", "sha1", "sha256", "sha512"], 
                        default="md5", help="Hash function to use")
    ...
    parser.add_argument("--division-method", type=str, 
                        default="feature", help="division measure to use")
    ...
    if args.defense_method in ["GraphGuard", "GG"]:
        ...
        if args.division_method == 'all':
            print(args.dataset, end=',')
            print(args.num_group)
            max_radius = 2 + args.num_group // 2
            result_matrix = np.zeros((max_radius, max_radius))
            for (feature_perturbation, structure_perturbation), value in np.ndenumerate(result_matrix):
                size = (feature_perturbation + structure_perturbation) * args.num_group - feature_perturbation * structure_perturbation
                result_matrix[feature_perturbation, structure_perturbation] = np.sum(pred & (Mp > size)) / len(Mp)
            np.set_printoptions(precision=4, linewidth=200)
            print(repr(result_matrix))                              
        else:
            print(args.dataset, end=',')
            print(args.num_group, end=',')
            print(args.hash_method, end=',')
